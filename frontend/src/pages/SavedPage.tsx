...
export default function SavedPage() {
  ...
  const handleLoad = (saved: { id: number; name: string; resume: unknown; template: string }) => {
    loadResume(saved as { resume: any; template: any });
    ...
    <div className="min-h-screen pt-24 px-4 pb-16 max-w-3xl mx-auto">
      ...
      {savedResumes.length === 0 ? (
        <div className="card p-16 text-center">
          ...
          <button onClick={() => navigate('/builder')} className="btn-primary text-sm mx-auto">Start Building</button>
        </div>
      ) : (
          ...
          {savedResumes.map((saved) => (
            <div key={saved.id} className="card p-4 flex items-center justify-between gap-4 hover:shadow-md transition-shadow">
              ...
              <div className="flex items-center gap-2 shrink-0">
                <button onClick={() => handleLoad(saved)} className="btn-secondary text-xs py-1.5 px-3">
                  <ExternalLink size={12} /> Load
                </button>
                <button onClick={() => handleDelete(saved.id)}
